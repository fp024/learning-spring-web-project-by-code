...
public interface BoardMapper {
	...
	default int insert(BoardVO record) {
		return MyBatis3Utils.insert(this::insert, record, boardVO,
				c -> c.map(bno).toProperty("bno").map(title).toProperty("title").map(content).toProperty("content")
						.map(writer).toProperty("writer").map(regdate).toProperty("regdate").map(updateDate)
	...
	default int insertSelective(BoardVO record) {
		return MyBatis3Utils.insert(this::insert, record, boardVO,
				c -> c.map(bno).toProperty("bno").map(title).toPropertyWhenPresent("title", record::getTitle)
						.map(content).toPropertyWhenPresent("content", record::getContent).map(writer)
						.toPropertyWhenPresent("writer", record::getWriter).map(regdate)
						.toPropertyWhenPresent("regdate", record::getRegdate).map(updateDate)
						.toPropertyWhenPresent("updateDate", record::getUpdateDate));
	}
	...
	static UpdateDSL<UpdateModel> updateAllColumns(BoardVO record, UpdateDSL<UpdateModel> dsl) {
		return dsl.set(bno).equalTo(record::getBno).set(title).equalTo(record::getTitle).set(content)
				.equalTo(record::getContent).set(writer).equalTo(record::getWriter).set(regdate)
				.equalTo(record::getRegdate).set(updateDate).equalTo(record::getUpdateDate);
	}

	@Generated("org.mybatis.generator.api.MyBatisGenerator")
	static UpdateDSL<UpdateModel> updateSelectiveColumns(BoardVO record, UpdateDSL<UpdateModel> dsl) {
		return dsl.set(bno).equalToWhenPresent(record::getBno).set(title).equalToWhenPresent(record::getTitle)
				.set(content).equalToWhenPresent(record::getContent).set(writer).equalToWhenPresent(record::getWriter)
				.set(regdate).equalToWhenPresent(record::getRegdate).set(updateDate)
				.equalToWhenPresent(record::getUpdateDate);
	}

	@Generated("org.mybatis.generator.api.MyBatisGenerator")
	default int updateByPrimaryKey(BoardVO record) {
		return update(c -> c.set(title).equalTo(record::getTitle).set(content).equalTo(record::getContent).set(writer)
				.equalTo(record::getWriter).set(regdate).equalTo(record::getRegdate).set(updateDate)
				.equalTo(record::getUpdateDate).where(bno, isEqualTo(record::getBno)));
	}

	@Generated("org.mybatis.generator.api.MyBatisGenerator")
	default int updateByPrimaryKeySelective(BoardVO record) {
		return update(c -> c.set(title).equalToWhenPresent(record::getTitle).set(content)
				.equalToWhenPresent(record::getContent).set(writer).equalToWhenPresent(record::getWriter).set(regdate)
