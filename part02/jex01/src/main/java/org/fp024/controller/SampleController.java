...
public class SampleController {
   ...
   *
   * <p>객체형이 아닌 기본 형을 required = false 로 두고 관련 파라미터를 전달하지 않으면...
   *
   * <p>Optional int parameter 'age' is present but cannot be translated into a null value due to
   * being declared as a primitive type. Consider declaring it as object wrapper for the
   * corresponding primitive type.
   *
   * <p>기본타입 int를 null로 변환할 수 없어 오류가 나므로, 기본 타입을 객체 레퍼로 선언하는 것으로 고려하란 메시지의 예외가 발생함.
   */
  @GetMapping("/ex02")
  public String ex02(
   ...
   *
   * <p>- 이름을 중복해서 여러번 보내도 된다. http://localhost:8080/jex01/sample/ex02List?ids=111&ids=222&ids=333
   */
  @GetMapping("/ex02List")
   ...
   * 6.3.3 객체 리스트
   *
   * <p>http://localhost:8080/jex01/sample/ex02Bean?list[0].name=aaa&list[0].age=1
   *
   * <p>* 톰켓 9.0에서 아래 오류 발생 java.lang.IllegalArgumentException: Invalid character found in the
   * request target [/ex01/sample/ex02Bean?list[0].name=aaa&list[0].age=1]. The valid characters are
   * defined in RFC 7230 and RFC 3986
   *
   * <p>톰켓에서 해당 문자를 허용하지 않아서 그렇다는데.. catalina.properties 파일에 아래 내용 추가하면 허용해줄 수 있으나...
   * tomcat.util.http.parser.HttpParser.requestTargetAllow=[]
   *
   * <p>URI 인코드 해서 보내라함...
   *
   * <p>encodeURI("http://localhost:8080/jex01/sample/ex02Bean?list[0].name=aaa&list[0].age=1&list[1].name=bbb&list[1].age=2&list[4].name=bbb")
   *
   * <p>==>
   ...
   *
   * <p>* (참고) 3번째 요소를 넘기고 4번째 요소의 이름만 입력했을 때, 3번째는 빈 객체로 들어간다. - list dtos:
   * SampleDTOList(list=[SampleDTO(name=aaa, age=1), SampleDTO(name=bbb, age=2),
   * SampleDTO(name=null, age=0), SampleDTO(name=null, age=0), SampleDTO(name=bbb, age=0)])
   *
   * <p>참조:
  ...
  /**
   * Spring MVC의 컨트롤러는 기본적으로 Java Beans 규칙에 맞는 객체는 다시 화면으로 객체를 전달함. (그동안 정확히 몰랐음.. 그동안 항상 model
   * attribute로 추가해줬었음. ㅠㅠ)
   *
   * <p>http://localhost:8080/jex01/sample/ex04?page=1&name=aaa&age=10 ==> 기본형으로 전달된 page 정보는 출력되지
   * 않지만, SampleDTO에 대한 정보가 JSP에 출력됨.
   *
   * <p>그러나 기본형 타입 파라미터라도 @ModelAttribute 를 붙여사용하면, 무조건 Model에 담아서 전달하므로, 파라미터로 전달된 데이터를 다시 화면에서
   * 사용해야할 경우 유용하게 사용됨.
   */
  @GetMapping("/ex04")
   ...
   *
   * <p>* 일회성으로 데이터를 전달하는 용도로 사용.
   */

  /**
   * 6.5.1 void 타입
   *
   * <p>InternalResourceViewResolver에 정의된대로 /WEB-INF/views/sample/ex05.jsp 를 찾아감.
   */
  @GetMapping("/ex05")
  public void ex05() {
    LOGGER.info("/ex05 ..........");
   ...
   * 6.5.3 객체 타입
   *
   * <p>http://localhost:8080/ex01/sample/ex06
   *
   * <p>==>
   *
   * <p>{ "name": "홍길동", "age": 10 }
   ...
   * 6.5.5 파일 업로드 처리
   *
   * <p>http://localhost:8080/jex01/sample/exUpload
   */
  @GetMapping("/exUpload")
  ...
  /** 에러 페이지 접근 테스트 ==> http://localhost:8080/jex01/sample/ex04?name=aaa&age=a */
}
