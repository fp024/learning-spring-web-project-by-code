...
public class SampleController {
   ...
   *
   * <p>* 이름을 중복해서 여러번 보내도 된다. http://localhost:8080/ex01/sample/ex02List?ids=111&ids=222&ids=333
   */
  @GetMapping("/ex02List")
   ...
   * 6.3.3 객체 리스트
   *
   * <p>http://localhost:8080/ex01/sample/ex02Bean?list[0].name=aaa&list[0].age=1
   *
   ...
   * <p>URI 인코드 해서 보내라함...
   *
   * <p>encodeURI("http://localhost:8080/ex01/sample/ex02Bean?list[0].name=aaa&list[0].age=1&list[1].name=bbb&list[1].age=2&list[4].name=bbb")
   *
   * <p>==>
   ...
   *
   * <p>http://localhost:8080/ex01/sample/ex04?page=1&name=aaa&age=10 ==> 기본형으로 전달된 page 정보는 출력되지
   ...
   * 6.5.5 파일 업로드 처리
   *
   * <p>로딩은 했지만 타겟 경로로 저장하는 일은 구현하지 않았다
   */
  @GetMapping("/exUpload")
  ...
  public String exUploadPost(List<MultipartFile> files) {
    ...
    return "redirect:exUpload"; // 실제 업로드 기능은 나중 파트에서 배우게된다고 함, 지금은 다시 업로드 페이지로 돌아가게만 바꿔두자!
  }

  /** 에러 페이지 접근 테스트 ==> http://localhost:8080/ex01/sample/ex04?name=aaa&age=a */
}
